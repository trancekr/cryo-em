...
(function() {
  ...
  function ensureAbsolute(el) {
    ...
    if (parent) {
      ...
      parent.style.minHeight = pRect.height + 'px';
      parent.style.minWidth = pRect.width + 'px';
    ...
    targets.forEach(({ el: target, rect }) => {
      ...
      target.style.left = Math.round(rect.left - cRect.left - bL) + 'px';
      target.style.top = Math.round(rect.top - cRect.top - bT) + 'px';
  ...
  function updateInfoBar() {
    ...
    infoBar.querySelector('.ib-size').textContent = `${Math.round(r.width)}×${Math.round(r.height)}`;
